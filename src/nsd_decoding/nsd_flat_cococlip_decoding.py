...
def load_split_tensors(ds, mask, subs=None, avgpool=False):
    ...
    if avgpool:
        ...
        for sub_id, nsd_id in np.unique(sample_ids, axis=0):
            idx_mask = (sub_ids == sub_id) & (nsd_ids == nsd_id)
            activity[idx_mask] = activity[idx_mask].mean(axis=0, keepdim=True)
...
def main(args):
    ...
    for split, (_, targets) in splits.items():
        ...
        acc = 100 * accuracy_score(targets.cpu().numpy(), preds)
