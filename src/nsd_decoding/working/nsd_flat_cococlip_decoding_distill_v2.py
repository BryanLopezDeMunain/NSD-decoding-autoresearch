"""distill_v2: Within-subject soft distillation from CLIP logits with KL divergence."""
...
def load_clip_soft_targets(clip_logits_all, categories_path, temperature):
    """Precompute soft targets from CLIP logits restricted to the 24 target classes."""
    with open(categories_path) as f:
        cats = json.load(f)
    coco_ids = list(cats.values())
    clip_logits_24 = clip_logits_all[:, coco_ids]
    soft_targets = torch.tensor(clip_logits_24, dtype=torch.float32)
    soft_targets = F.softmax(soft_targets / temperature, dim=-1)
    return soft_targets


def load_split_tensors(ds, mask, subs, soft_targets):
    ...
    split_soft = soft_targets[nsd_ids]

    class_targets = torch.tensor(np.array(ds["target"]), dtype=torch.long)
    return activity, split_soft, class_targets
...
def train_one_epoch(model, loader, optimizer, criterion):
    ...
    for x, soft_y in loader:
        ...
        loss.backward()
        optimizer.step()
        total_loss += loss.item() * soft_y.size(0)
...
def main(args):
    ...
    # Load CLIP logits and compute soft targets
    clip_logits_all = np.load(ROOT / "datasets/nsd_clip_coco_logits.npy")
    soft_targets = load_clip_soft_targets(
        clip_logits_all, ROOT / "metadata/nsd_cococlip_categories.json", args.temperature
    )
    print(f"CLIP logits: {clip_logits_all.shape} -> soft targets: {soft_targets.shape}")
    ...
    for name, hf_split in split_map.items():
        act, soft, tgt = load_split_tensors(dataset_dict[hf_split], mask, subs, soft_targets)
        splits[name] = (act.to(device), soft.to(device), tgt.to(device))
        print(f"  {name} ({hf_split}): {act.shape}, soft: {soft.shape}, targets: {tgt.shape}")
    ...
    eval_loaders = {
        ...
        for split, (act, soft, tgt) in splits.items()
    }
